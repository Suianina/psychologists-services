  ...
  useEffect(() => {
    const fetchPsychologists = async () => {
      ...
      try {
        ...
        console.log("🔥 Firebase data:", rootSnapshot.val()); // ← ДОДАТИ

        if (rootSnapshot.exists()) {
          const data = rootSnapshot.val();
          console.log("✅ Data exists:", data); // ← ДОДАТИ
          ...
          console.log("📋 Psychologists list:", list); // ← ДОДАТИ

          setAllPsychologists(list);
        } else {
          console.log("❌ No data found"); // ← ДОДАТИ
        }
      } catch (error) {
          ...
          <div className={styles.list}>
            {visiblePsychologists.map((psychologist) => (
              <PsychologistCard
                ...
                onToggleFavorite={() => handleFavoriteToggle(psychologist.id)}
              />
            ))}
          </div>
        )}
